...
<div class="admin-card-body custom-scrollbar bg-slate-50/20 px-6 py-4 space-y-8">
    ...
    <div class="flex items-end gap-4">
        ...
        <div class="flex items-center gap-2 border-l border-slate-200 pl-4 h-10 shrink-0">
            <label class="toggle-switch">
                ...
                <span
                    class="ml-3 font-bold text-sm w-24 block text-left {{ $concern->is_redirect ? 'text-admin-blue' : 'text-slate-600' }}"
                    id="redirectLabel">
    ...
    <div>
        <label class="text-[11px] font-bold text-slate-400 uppercase ml-1 block mb-2">Cover Photo (20:9)</label>
        ...
        @if($concern->cover_photo_path)
            @php
                ...
                $coverThumb = dirname($concern->cover_photo_path) . '/thumbs/' . $coverBase . '-700.webp';
            @endphp
            <div
                class="relative group w-full aspect-20/9 rounded-2xl overflow-hidden border border-slate-200 shimmer bg-slate-100">
                <img src="{{ asset('storage/' . $coverThumb) }}?v={{ time() }}"
        ...
        @else
            <div class="w-full aspect-20/9 bg-white border-2 border-dashed border-slate-300 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-admin-blue group hover:bg-slate-50 transition-all"
                onclick="document.getElementById('coverUploadInput').click()">
                <i
                ...
                <span
                    ...
                    to select 20:9 image</span>
            </div>
        @endif
    </div>

    <div>
        <label class="text-[11px] font-bold text-slate-400 uppercase ml-1 block mb-2">Photo Gallery (20:9)</label>
        ...
        <div class="w-full aspect-20/9 bg-white border-2 border-dashed border-slate-300 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-admin-blue group hover:bg-slate-50 transition-all mb-4"
        ...
        @if($galleries->count() > 0)
            <div id="gallery-sortable" class="grid grid-cols-2 gap-4">
                @foreach($galleries as $g)
                    @php
                        $galleryBase = basename($g->file_path, '.webp');
                        $galleryThumb = dirname($g->file_path) . '/thumbs/' . $galleryBase . '-350.webp';
                    @endphp
                    <div class="sortable-gallery-item relative group w-full aspect-20/9 rounded-xl overflow-hidden border border-slate-200 bg-slate-100 shimmer"
                        data-id="{{ $g->id }}">
                        <div
                        ...
                        <img src="{{ asset('storage/' . $galleryThumb) }}?v={{ time() }}"
                            class="w-full h-full object-cover opacity-0 transition-opacity duration-300"
                            onload="this.classList.remove('opacity-0'); this.parentElement.classList.remove('shimmer')">
                        <div class="absolute inset-0 bg-admin-blue/60 opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer transition-all backdrop-blur-sm"
    ...
    <form id="descForm" onsubmit="submitDesc(event, {{ $menu->id }})" class="pb-2">
        <div class="mb-2">
            <label class="text-[11px] font-bold text-slate-400 uppercase ml-1 block">Description</label>
        </div>
        <textarea id="concernDesc" name="description" class="hidden">{{ $concern->description }}</textarea>
        <div class="flex justify-end mt-4">
            <button type="submit" class="btn-primary h-10 px-6">Update Description</button>
        </div>
    </form>
