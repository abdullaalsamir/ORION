<div id="contactModal" class="modal-overlay hidden">
    <div class="modal-content max-w-xl! h-[90vh]! flex flex-col">
        ...
        <div class="flex justify-between items-center pb-3 border-b border-slate-100 shrink-0">
            ...
            <button type="button" onclick="closeModal('contactModal')" class="btn-icon">
                <i class="fas fa-times text-xl"></i>
            </button>
        </div>

        <form id="contactForm" action="{{ route('admin.footer.update') }}" method="POST"
            ...
            <div class="flex flex-col gap-2">
                ...
                <div class="relative">
                    <input type="text" name="address_1" value="{{ $footer->address_1 }}" placeholder="Address Line 1"
                        required maxlength="50" class="input-field w-full mb-2"
                        oninput="updateCount(this, 'c_addr1', 50)">
                    <span id="c_addr1" class="absolute right-3 top-2.5 text-[9px] text-slate-300 font-bold">0/50</span>
                </div>
                <div class="relative">
                    <input type="text" name="address_2" value="{{ $footer->address_2 }}" placeholder="Address Line 2"
                        required maxlength="50" class="input-field w-full mb-2"
                        oninput="updateCount(this, 'c_addr2', 50)">
                    <span id="c_addr2" class="absolute right-3 top-2.5 text-[9px] text-slate-300 font-bold">0/50</span>
                </div>
                <div class="relative">
                    <input type="text" name="address_3" value="{{ $footer->address_3 }}" placeholder="Address Line 3"
                        required maxlength="50" class="input-field w-full" oninput="updateCount(this, 'c_addr3', 50)">
                    <span id="c_addr3" class="absolute right-3 top-2.5 text-[9px] text-slate-300 font-bold">0/50</span>
                </div>
            </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                <div class="relative">
                    <input type="text" name="phone_1" value="{{ $footer->phone_1 }}" placeholder="Phone Number 1"
                        required maxlength="50" class="input-field w-full mb-2"
                        oninput="updateCount(this, 'c_ph1', 50)">
                    <span id="c_ph1" class="absolute right-3 top-2.5 text-[9px] text-slate-300 font-bold">0/50</span>
                </div>
                <div class="relative">
                    <input type="text" name="phone_2" value="{{ $footer->phone_2 }}" placeholder="Phone Number 2"
                        required maxlength="50" class="input-field w-full" oninput="updateCount(this, 'c_ph2', 50)">
                    <span id="c_ph2" class="absolute right-3 top-2.5 text-[9px] text-slate-300 font-bold">0/50</span>
                </div>
            </div>
        ...
        <div class="flex justify-end items-center border-t border-slate-100 pt-4 shrink-0 bg-white">
            <button type="submit" form="contactForm" class="btn-primary h-10">
                Update Profile
            </button>
        </div>
    </div>
...
<div id="mapModal" class="modal-overlay hidden">
    <div class="modal-content max-w-2xl! h-[90vh]! flex flex-col">
        ...
        <div class="flex justify-between items-center pb-3 border-b border-slate-100 shrink-0">
            ...
            <button type="button" onclick="closeModal('mapModal')" class="btn-icon">
                <i class="fas fa-times text-xl"></i>
            </button>
        </div>

        <form id="mapForm" action="{{ route('admin.footer.update') }}" method="POST"
            class="flex-1 overflow-y-auto custom-scrollbar pr-2 py-6 flex flex-col space-y-6">
            @csrf
            <div class="flex flex-col gap-1 shrink-0">
                ...
                <div class="flex gap-2">
                    ...
                    <button type="button" onclick="fetchFooterMap()" class="btn-primary h-11 px-6 bg-slate-800">
                        Fetch Map
                    </button>
                </div>
            </div>
        ...
        <div class="flex justify-end items-center border-t border-slate-100 pt-4 shrink-0 bg-white">
            <button type="submit" form="mapForm" id="mapSaveBtn" disabled
                class="btn-success h-10 opacity-50 cursor-not-allowed">
                Update Map
            </button>
        </div>
    </div>
...
<div id="qlModal" class="modal-overlay hidden">
    <div class="modal-content max-w-xl! h-[90vh]! flex flex-col">
        ...
        <div class="flex justify-between items-center pb-3 border-b border-slate-100 shrink-0">
            ...
            <button type="button" onclick="closeModal('qlModal')" class="btn-icon">
                <i class="fas fa-times text-xl"></i>
            </button>
        </div>

        <form id="qlForm" action="{{ route('admin.footer.update') }}" method="POST"
            class="flex-1 overflow-y-auto custom-scrollbar pr-2 py-6">
            @csrf
            <div id="ql-sortable" class="space-y-3">
                @for($i = 0; $i < 8; $i++)
                    @php $current = $footer->quick_links[$i] ?? null; @endphp
                    <div class="sortable-item group bg-slate-50 border border-slate-200 rounded-2xl p-2 flex items-center">
                        <div
                        ...
                        <select name="ql_menu_id[]" class="input-field h-11! flex-1 ql-select ml-2">
        ...
        <div class="flex justify-end items-center border-t border-slate-100 pt-4 shrink-0 bg-white">
            <button type="submit" form="qlForm" class="btn-primary h-10">
                Update Slots
            </button>
        </div>
    </div>
...
<div id="followModal" class="modal-overlay hidden">
    <div class="modal-content max-w-xl! h-[90vh]! flex flex-col">
        ...
        <div class="flex justify-between items-center pb-3 border-b border-slate-100 shrink-0">
            ...
            <button type="button" onclick="closeModal('followModal')" class="btn-icon">
                <i class="fas fa-times text-xl"></i>
            </button>
        </div>

        <form id="followForm" action="{{ route('admin.footer.update') }}" method="POST"
            class="flex-1 overflow-y-auto custom-scrollbar pr-2 py-6 space-y-6">
            @csrf
            ...
            <div class="flex flex-col gap-1 relative">
                ...
                <textarea name="follow_us_desc" required maxlength="200"
                    class="input-field w-full h-28 py-4 resize-none custom-scrollbar"
            ...
            <div class="flex flex-col gap-1">
                <label class="text-[11px] font-bold text-slate-400 uppercase ml-1 mb-2">Social Links
                    Configuration</label>
                <div id="social-sortable" class="space-y-3">
                    @foreach($footer->social_links ?? [] as $social)
                        <div
                            class="sortable-item group bg-slate-50 border border-slate-200 rounded-2xl p-2 flex items-center">
                            <div
                                class="drag-handle w-8 flex justify-center cursor-grab active:cursor-grabbing text-slate-300 hover:text-admin-blue transition-colors">
                                <i class="fas fa-arrows-up-down-left-right"></i>
                            </div>

                            <span class="w-20 text-xs font-semibold text-slate-500 uppercase text-center">
                                {{ $social['platform'] }}
                            </span>
                            ...
                            <input type="text" name="social_url[]" value="{{ $social['url'] }}" placeholder="https://..."
                                class="input-field h-10! flex-1 text-xs! font-medium! ml-2">
                        </div>
                    @endforeach
                </div>
            </div>
        ...
        <div class="flex justify-end items-center border-t border-slate-100 pt-4 shrink-0 bg-white">
            <button type="submit" form="followForm" class="btn-primary h-10">
                Update Branding
            </button>
        </div>
    </div>
